/**
 * NXL Field Timer — ESP32 GREEN Button Box
 *
 * No WiFi — ESP-NOW only to Referee ESP32.
 * Automatically syncs to referee's WiFi channel via broadcast.
 *
 * GPIO 13 → SHORT press: GREEN_SPACE
 * GPIO 27 → GREEN_BTN   (point RED / green T/O)
 * GPIO 12 → GREEN_SURRENDER
...
void addRefereePeer(int channel){
  esp_now_del_peer(REFEREE_MAC); // remove old if exists
...
void onRecv(const esp_now_recv_info_t* info, const uint8_t* data, int len){
  // Receive channel broadcast from referee
...
void initEspNow(){
  ...
  // Start on channel 1, will auto-correct when referee broadcasts
...
void setup(){
  ...
  Serial.printf("[MAC] GREEN: %s\n", WiFi.macAddress().c_str());
  pinMode(PIN_SPACE, INPUT_PULLUP);
  pinMode(PIN_BTN_A, INPUT_PULLUP);
  pinMode(PIN_BTN_B, INPUT_PULLUP);
  pinMode(PIN_RESET, INPUT_PULLUP);
  initEspNow();
  Serial.println("[READY] GREEN ESP32");
}
...
void loop(){
  ...
  if(now - lastHB > 4000){ lastHB = now; sendCmd("GREEN_ALIVE"); }
  ...
  if(spLow&&!spHeld){spHeld=true;spTime=now;}
    ...
    spHeld=false;
    if(now-spTime>DEBOUNCE_MS) sendCmd("GREEN_SPACE");
  }

  // GPIO 27 & 12 with cooldown
  bool cool=(now<scoreCooldownUntil);
  struct{int pin;const char*cmd;}btns[]={{PIN_BTN_A,"GREEN_BTN"},{PIN_BTN_B,"GREEN_SURRENDER"}};
